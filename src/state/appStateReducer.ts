...
export const appStateReducer = (
  ...
  switch (action.type) {
    ...
    case "DELETE_TASK": {
      ...
      const sourceListIndex = findItemIndexById(draft.lists, columnId);
      let taskIndex = findItemIndexById(
        draft.lists[sourceListIndex].tasks,
        listId
      );

      // Remove the task from the source list
      draft.lists[sourceListIndex].tasks.splice(taskIndex, 1);
      break;
    }
